<?php 
	...
	{
	   $sql = mysqli_query($CONNECTION, "DELETE FROM cookies WHERE BINARY cookie_value = '".$_COOKIE['account']."'");
	...
	if (isset($_POST['name']) && isset($_POST['surname']) && isset($_POST['email']) && isset($_POST['username']) && isset($_POST['password']) && isset($_POST['passwordConfirm']) && isset($_POST['lang']) && isset($_POST['currency']) && isset($_POST['city']) && isset($_POST['state']))
	{
		if (!isset($_COOKIE['account'])) 
		{
			$sql = mysqli_query($CONNECTION, "INSERT INTO users (name, surname, email, username, password, lang, currencyID, security_token, register_date, last_login, access_token, timestamp, city, country) VALUES (
				'".$_POST['name']."', '".$_POST['surname']."', 
				'".$_POST['email']."', '".$_POST['username']."', '".$_POST['password']."', '".$_POST['lang']."',
				'".$_POST['currency']."',
				'".($tok = generate_string(30))."', CURDATE(), CURDATE(), '".($atok = generate_string(20))."', ".time().",
				'".htmlspecialchars($_POST['city'], ENT_QUOTES)."', '".htmlspecialchars($_POST['state'], ENT_QUOTES)."'
				)");
			if ($sql)
			{
...
//				send_email();
				...
				// Handle IP address
//				$ip = "178.220.249.89"; //getUserIP();
//				$resp = json_decode(file_get_contents("http://freegeoip.net/json/".$ip));
//				$sql = mysqli_query($CONNECTION, "INSERT INTO ipaddress(ip_address, date, timestamp, country_code, country_name, region_name, city, zip_code, timezone, latitude, longitude, metro_code, username) VALUES ('".$ip."', CURDATE(), '".time()."', '".$resp->country_code."', '".$resp->country_name."', '".$resp->region_name."', '".$resp->city."', '".$resp->zip_code."', '".$resp->timezone."', '".$resp->latitude."', '".$resp->longitude."', '".$resp->metro_code."', '".$username."')");
				...
				<div class="login-placeholder">
						...
						<div class="smaller">
							<div class="padding-l-40 padding-md-l-30 padding-t-5">
								<ol class="ordered-list-errors">									
									...
									<li class="dn"><?php echo $lang->reg_cityErr ?></li>
									<li class="dn"><?php echo $lang->reg_stateErr ?></li>
								</ol>
							</div>
						</div>
					</div>
					<form id="registerForm" method="post" action="<?= $urlToPostData; ?>">
						...
						<div class="row">
							<div class="col-md-6">
								<div class="login-input-group margin-t-10">
									<input id="city" type="text" class="input-theme" name="city" placeholder="<?= $lang->city; ?>" autocomplete="off" />
									<div class="login-input-icon"><i class="fa fa-map-marker"></i></div>
								</div>
							</div>
							<div class="col-md-6">
								<div class="login-input-group margin-t-10">
									<input id="state" type="text" class="input-theme" name="state" placeholder="<?= $lang->state; ?>" autocomplete="off" />
									<div class="login-input-icon"><i class="fa fa-map-marker"></i></div>
								</div>
							</div>						
						</div>
						...
						<div class="margin-t-20 margin-b-20 text-center">
							<button id="register" class="paper-btn"><i class="fa fa-sign-in"></i> 
