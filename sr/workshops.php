...
//  Set @REDIRECT@ parameter to ensure `page closing` on `user open attempt`
	...
	$sql = mysqli_query($CONNECTION, "SELECT packageID, text_".$lang_acr." AS text, price_".$curr." AS pprice, price_RSD AS ppriceRSD FROM packages");
	$i = 0; $PACKAGES = array();
					...
					<div class="pay-placeholder small">
						...
						<a href="<?= $FILE ?>user/addCart/subscription/<?= $PACKAGES[0]->packageID ?>?month=<?= $month ?>">
							<div class="btn-green btn-ord btn-block">
								Subscribe for <span class="strong"><?= $lang->c_months[$month-1] ?></span>
							</div>	
						</a>						
						<?php } else { ?>
