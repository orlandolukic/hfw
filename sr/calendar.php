<?php 
	...
	$setEvent = isset($_GET['eventID']) && trim($_GET['eventID']) !== "";
	if ($setEvent)
	{
		$sql = mysqli_query($CONNECTION, "SELECT date, location, heading_".$lang_acr." AS heading, text_".$lang_acr." AS text, MONTH(date) AS month, YEAR(date) AS year FROM events WHERE BINARY eventID = '".$_GET['eventID']."' AND active = 1");
	...
	<div class="main-container margin-md-t-60">
		<div class="welcome-note welcome-note-min welcome-artist">
			...
			<div class="content text-center">
				<div class="heading strong heading-special heading-special-rest md-heading-special padding-l-60 padding-r-60" style="display: inline-block;"> <?= $lang->calendar ?> </div>					
			</div>
		</div>
	</div>
	<!-- /Main Container -->
		...
		<div class="row">			
			<div class="col-md-6 col-xs-12 col-sm-12 margin-md-t-30">
				<div class="responsive-calendar op-0">
				  <div class="controls">
				      <a class="pull-left" data-go="prev" data-toggle="tooltip" data-placement="right" title="<?= $lang->prevMonth ?>"><div class="btn-initiator-send color-white"><i class="fa fa-chevron-left"></i></div></a>
				      <h4><span data-head-year></span> <span data-head-month></span> <i class="fa fa-calendar"></i></h4>
				      <a class="pull-right" data-go="next" data-toggle="tooltip" data-placement="left" title="<?= $lang->nextMonth ?>"><div class="btn-initiator-send color-white"><i class="fa fa-chevron-right"></i></div></a>
				  </div><hr/>
				...
				<div class="calendar-event-wrapper padding-t-10">
					<div class="event-wrapper margin-t-10">
						...
						<div class="margin-t-20">
							<div id="evBackLst" class="btn-initiator-send small"><i class="fa fa-chevron-left"></i> <?= $lang->backEvents ?></div>
						</div>
					</div>
