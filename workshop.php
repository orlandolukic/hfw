...
// 	SQL STATEMENTS
	$sql = mysqli_query($CONNECTION, "SELECT workshopID, heading_".$lang_acr." AS heading, subheading_".$lang_acr." AS subheading, forsale, active, date_publish, date_end, text_".$lang_acr." AS text, MONTH(date_publish) AS start_month, sale, sale_date_start, sale_date_end, price_".$curr." AS price, price_RSD as priceRSD, views FROM workshops WHERE (BINARY workshopID = '$wid' OR BINARY heading_".$lang_acr." = '$wid') AND active = 1 AND date_publish<=CURDATE() LIMIT 1");
	...
	if ($userActive) $sql_subscription = mysqli_query($CONNECTION, "SELECT * FROM subscriptions WHERE BINARY username = '".$USER->username."' AND active = 1 AND date_start<=CURDATE() LIMIT 1");
		...
		<div class="container">
			<div class="row">
				...
				<div class="col-md-7 col-xs-12 col-sm-12 margin-md-t-30">
					...
					//	Sale
					if ($workshop->sale && !(
                            $passCheck && $passCheck->bought
                            || $passCheck && $passCheck->subscriptions
                            || $userActive && $USER->grant_access))
					{
					...
					<div class="workshop-sale margin-t-20 margin-b-30">
						...
						<script type="text/javascript" src="<?= $FILE ?>js/csscript.js"></script>
					</div>
					<?php
					}
					// End of section Sale
					...
					<div class="margin-b-20">
						...
						<div class="large div-inl">
							<?php if ($userActive) { ?>
							<?php if ((!$passCheck->bought && !$passCheck->subscriptions || !$passCheck->subscriptions && !$passCheck->bought) && !$USER->grant_access) { ?>
							<div id="addToWishList" data-workshop-id="<?= $wid ?>" data-after-mssg="<?= $lang->alreadyOnList ?>" data-success-mssg="<?= $lang->succAddWishLst ?>" class="pointer" data-set="<?= (mysqli_num_rows_wrapper($sql_wishlist) ? "true" : "false"); ?>">
								<i class="fa fa-heart icon icon-heart<?= (mysqli_num_rows_wrapper($sql_wishlist) ? " icon-heart-selected" : ""); ?>" data-toggle="tooltip" title="<?= (mysqli_num_rows_wrapper($sql_wishlist) ? $lang->alreadyOnList : $lang->addToWish); ?>" data-placement="bottom"></i>
							</div>
							<?php }; ?>

							<div class="pointer padding-l-10">
								<a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=http%3A%2F%2Fhandmadefantasyworld.com/workshop/<?= $workshop->workshopID ?>" data-toggle="tooltip" title="<?php echo $lang->share_facebook ?>" data-placement="bottom">
							...
							<div class="pointer">
								<a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=http%3A%2F%2Fhandmadefantasyworld.com/workshop/<?= $workshop->workshopID ?>" data-toggle="tooltip" title="<?= $lang->share_facebook ?>" data-placement="bottom">
								<i class="fa fa-facebook-official icon icon-facebook"></i>
								</a>
							</div>
							<?php }; ?>							
							<a href="https://twitter.com/intent/tweet?text=Handmade Fantasy World <?= $lang->workshopUP ?> - <?= $workshop->heading ?> <?= $domain ?>workshop/<?= $workshop->workshopID ?>" target="_blank" style="display: inline-block;" data-toggle="tooltip" title="<?php echo $lang->share_twitter ?>" data-placement="bottom">
								<div class="pointer padding-l-10"><i class="fa fa-twitter icon icon-twitter"></i></div>
							</a>
						</div>
					</div>
					...
					<?php 
					  ...
					  if ($userActive) 
					  { 						
						if ($subscrSection = (bool) mysqli_num_rows_wrapper($sql_subscription)) {
						...
						<div class="margin-t-20">
							...
							<a href="<?= $FILE ?>user/video/<?= $workshop->workshopID ?>">
								<div class="btn-green btn-ord margin-t-10">
			...
			<?php   } elseif (!$subscrSection && !$printed || !$passCheck) {					
						$i = 0;	while($t = mysqli_fetch_object_wrapper($sql_packages)) $packages[$i++] = $t;	?>				

					<div class="div-inl margin-t-10">						
						<div id="price" class="strong div-inl">
					...
					<div class="margin-t-10 md-text-center">
						<?php if ($userActive) { 						
						?>
						<a id="subscribeLink" href="<?= $FILE; ?>user/addCart/subscription?month=<?= $workshop->start_month ?>&cid=1&wid=<?= $workshop->workshopID ?>">
							<div id="subscribe" class="btn-green btn-ord" style="pointer-events: auto;"><i class="fa fa-shopping-cart"></i> <?php echo $lang->subscribe_for ?> <span class="strong"><?= $lang->c_months[$workshop->start_month-1] ?></span></div>
						</a>
						<?php } else { ?>
						<a id="subscribeLink" href="<?= $domain; ?>login.php?action=addCart&type=subscription&cid=1&month=<?= $workshop->start_month ?>&wid=<?=  $workshop->workshopID ?>">
							<div id="subscribe" class="btn-green btn-ord" style="pointer-events: auto;"><i class="fa fa-shopping-cart"></i> <?php echo $lang->subscribe_for ?> <span class="strong"><?= $lang->c_months[$workshop->start_month-1] ?></span> </div>
						</a>
						<?php }; ?>						
					</div>																
					    	...
					    	<div class="workshop-text">					    		
				    			<div class="row">
				    				...
				    				<div class="col-md-4 col-xs-12 col-sm-12 margin-md-t-20">
				    					<h4 style="margin: 0"><div class="strong uppercase l-sp-1"><?php echo $lang->ingredients ?></div></h4>
				    					<div class="margin-t-10">
											<?php for ($i=0, $tempID = -1, $margins = false, $print_free = true; $i<count($ingredients); $i++, $margins = true) { // Print ingredients ?>
													...
													<a href="javascript: void(0)" target="_blank" data-toggle="tooltip" data-placement="right"><span class="smaller bg-info padding-l-10 manufacturer paper-btn" style="cursor: not-allowed; padding: 10px 20px; color: black">Other supplies</span></a>
													<?php }; ?>
													<?php if ($ingredients[$i]->manufacturerID) { ?>
													<div class="<?= ($i>0 ? "margin-t-20" : "") ?>">
														<?php if ($ingredients[$i]->m_website) { ?>
														<a href="<?= $FILE ?>l.php?type=<?= base64_encode("manufacturer"); ?>&manufacturerID=<?= $ingredients[$i]->m_id ?>&redirect_url=<?= $ingredients[$i]->m_website ?>" target="_blank" data-toggle="tooltip" data-placement="right" title="Visit <?= $ingredients[$i]->m_name ?> website"><span class="small strong bg-info padding-l-10 paper-btn" style="padding: 10px 20px"><?= $ingredients[$i]->m_name ?></span></a>
														<?php } else { ?>
														<a href="javascript: void(0)" target="_blank" data-toggle="tooltip" data-placement="right"><span class="smaller bg-info padding-l-10 manufacturer paper-btn" style="cursor: not-allowed; padding: 10px 20px; color: black"><?= $ingredients[$i]->m_name ?></span></a>
														<?php }; ?>
													</div>
													<?php }; ?>
													<?php }; ?>
													<div>
														<span class="small"><?= $ingredients[$i]->text ?></span>								
						 	...
						 	<?php for ($i=0; $i<count($reviews); $i++) { ?>	
						    		...
						    		if ($userActive && $reviews[$i]->username===$user || $adminActive) { ?>
						    					...
						    					<?php };
						    						...
						    						//$pom_sql = mysqli_query($CONNECTION,"SELECT * FROM reviews WHERE ");
						    					...
						    					$sql_comments = mysqli_query($CONNECTION, "SELECT * FROM comments WHERE reviewID='".$reviews[$i]->reviewID."' ORDER BY time DESC, date DESC LIMIT 5");
							    				...
							    				<div class="row margin-md-t-20 margin-t-10 small">
							    					<div class="col-md-5 col-xs-12 col-sm-12 padding-l-70 padding-md-l-10">
							    						<div class="btn-fix user-comment-all-comments div-inl">
							    							<?php if (($adminActive && $ADMIN->access_workshop_comments) || ($userActive && mysqli_num_rows_wrapper($sql_comments))) { ?>
							    						 	<div>
							    							...
							    							<?php };				    							
							    							// Subquery
							    							$sql = mysqli_query($CONNECTION,"SELECT * FROM comments WHERE reviewID='".$reviews[$i]->reviewID."'");
							    							?>
							    							<div>
							    				...
							    				<div class="row">
							    					...
							    					<div class="col-md-6 col-xs-12 col-sm-12 border-r-shadow-eff border-b-shadow-eff pre-allComments" style="padding: 0">
							    						...
							    						<div class="allComments" data-comments-num="<?= mysqli_num_rows_wrapper($sql_comments) ?>">
							    							...
							    							<?php $go++; }; }; 
							    							$gr = mysqli_num_rows_wrapper(mysqli_query($CONNECTION, "SELECT * FROM comments WHERE reviewID='".$reviews[$i]->reviewID."'"))>5; ?>
							    						</div>
